...
void plotVesselbuTubeFilter()
{
	...
	fopen_s(&ImageA, "ImageA2.txt", "rb");//打开存储图片A二维点坐标的txt文件
	...
	{
		fscanf_s(ImageA, "%lf %lf", &ImageAx, &ImageAy);//按格式读取数据
		//将临时变量的值赋给二维结构体的相应对象
	...
	fopen_s(&ImageB, "ImageB2.txt", "rb");//打开存储图片B二维点坐标的txt文件
	...
	{
		fscanf_s(ImageB, "%lf %lf", &ImageBx, &ImageBy);//按格式读取数据
		//将临时变量的值赋给二维结构体的相应对象
	...
	for (int i = 0; i < ImageApoint.size() - 1; i++)
	{
		...
		ofstream f;
		f.open("三维坐标2.txt", ios::out | ios::app);
